...
class ComplexityClass(object):
    ...
    def fit(
        ...
        # NEW: residual mode for better stability
        residual_mode="relative",  # "sse" | "relative" | "log"
        eps=1e-12,
    ):
        """
        residual_mode:
          - "sse": sum (ref - t)^2  (old behavior)
          - "relative": sum ((ref - t)/(t+eps))^2  (better for growth curve fitting)
          - "log": sum (log(ref+eps) - log(t+eps))^2  (also robust; good when values span orders)
        """

        n = np.asanyarray(n)
        t = np.asanyarray(t)

        # original invariant used by upstream
        ...
        if apply_constraints:
            def inference_function(u_values, v_values):
                coeff, _ = scipy.optimize.nnls(u_values, v_values)
                return coeff
        else:
            def inference_function(u_values, v_values):
                coeff, _, rank, s = np.linalg.lstsq(u_values, v_values, rcond=-1)
                return coeff
        ...
        def wrapped_inference_function(u_values, v_values, zero_out_first_value, intercept_value):
            try:
                if zero_out_first_value:
                    ...
                    coeff = inference_function(u_values, v_values)
            except Exception:
                # fallback safe
                try:
                    if zero_out_first_value:
        ...
        # ---- piecewise handling (kept identical logic) ----
        ...
        # predicted curve in original t-space
        ref_t = self.compute(piecewise_x_list, n)

        # -------- NEW: robust residuals --------
        if residual_mode == "sse":
            residuals = float(np.sum((ref_t - t) ** 2))
        elif residual_mode == "log":
            # guard: ref_t could be slightly negative due to intercept; clamp
            rt = np.maximum(ref_t, 0.0)
            tt = np.maximum(t, 0.0)
            residuals = float(np.sum((np.log(rt + eps) - np.log(tt + eps)) ** 2))
        elif residual_mode == "relative":
            denom = np.maximum(np.abs(t), eps)
            residuals = float(np.sum(((ref_t - t) / denom) ** 2))
        else:
            raise ValueError(f"Unknown residual_mode: {residual_mode}")

        # keep last segment coeff for coeff reporting (same as before)
        self.piecewise_coeff_list = [self.piecewise_coeff_list[-1]]
        max_time = self.compute(
            [self._transform_n(np.array([max_time_x_value]))],
            np.array([max_time_x_value])
        )
...
class Logarithmic(ComplexityClass):
    def _transform_n(self, n):
        # Guard: log(0) is invalid; n in this benchmark should be >= 1
...
class Linearithmic(ComplexityClass):
    """
    O(n log n)
    inside_coef is kept from original code to avoid log(0) / small-argument artifacts.
    """
    inside_coef = 1.0
