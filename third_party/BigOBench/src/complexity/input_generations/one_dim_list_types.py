...
def list_int_base_random_verylarge(input_list):
    # keep magnitude "verylarge" but allow negative values to avoid degenerate all-positive lists
    return (
        'print([random.choice([-1, 1]) * random.randint(1000000, 1000000 * 10) for _ in '
        +
        input_list
        +
        '])'
...
def list_int_multiplier_random_verylarge(input_list, n):
    ...
    upper = max(n, 1000000 * 100)

    return (
        'print([random.choice([-1, 1]) * random.randint(1000000, {}) for x in ['.format(upper)
        +
        input_list
        +
        'for _ in range({})] for y in x] + '.format(n_1)
        ...
        input_list
        +
        ' + [random.choice([-1, 1]) * random.randint(1000000, {}) for x in ['.format(upper)
        +
        input_list
        +
        'for _ in range({})] for y in x])'.format(n_2)
