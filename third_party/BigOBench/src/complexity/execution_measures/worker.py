...
def main() -> None:
    ...
    try:
        ...
        if cpu_id_list is not None:
            assert current_process.cpu_affinity() == cpu_id_list
        # ============================
        # Robust aggregation (Warmup drop + Median + IQR)
        # Keep backward compatibility: do NOT remove value_list
        # Adds: value_med, value_iqr, n_eff
        # ============================
        WARMUP_DROP = int(data.get("warmup_drop", 1))  # allow override from main process later
        MIN_EFFECTIVE = int(data.get("min_effective_runs", 2))

        def _robust_summary(value_list, warmup_drop=WARMUP_DROP):
            """Return (median, iqr, n_eff) after dropping warmup runs."""
            ...
            for x in value_list:
                try:
                    ...
                    continue
            if len(v) <= warmup_drop:
                return None, None, 0
            vv = np.array(v[warmup_drop:], dtype=float)
            if vv.size == 0:
                return None, None, 0
            med = float(np.median(vv))
            iqr = float(np.percentile(vv, 75) - np.percentile(vv, 25)) if vv.size >= 2 else 0.0
            return med, iqr, int(vv.size)

        def _attach_stats_to_dict_list(dict_list, is_memory=False):
            """
            dict_list is runtime_dict_list OR memory_dict_list
            structure:
            dict_list[index][var_key][multiplier] -> list of dicts, each has 'value_list'
            """
            for idx in range(len(dict_list)):
                item = dict_list[idx]
                if not isinstance(item, dict):
                    continue
                for var_key, mult_dict in item.items():
                    if not isinstance(mult_dict, dict):
                        continue
                    for mult_k, runs in mult_dict.items():
                        if not isinstance(runs, list):
                            continue
                        for run_obj in runs:
                            if not isinstance(run_obj, dict):
                                continue
                            vlist = run_obj.get("value_list", None)
                            med, iqr, n_eff = _robust_summary(vlist)

                            # If too few effective runs, keep stats but mark as unreliable.
                            run_obj["n_eff"] = n_eff
                            run_obj["value_med"] = med
                            run_obj["value_iqr"] = iqr
                            run_obj["value_ok"] = bool(med is not None and n_eff >= MIN_EFFECTIVE)

        # runtime
        _attach_stats_to_dict_list(runtime_dict_list, is_memory=False)
        # memory (peak memory is int; median is still fine)
        _attach_stats_to_dict_list(memory_dict_list, is_memory=True)
